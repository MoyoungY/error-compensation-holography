...
def propagation_ASM(u_in, feature_size=feature_size, wavelength=wavelength, z=prop_dist, linear_conv=True,
    ...
    if precomped_H is None:
        ...
        H_filter = torch.tensor(((np.abs(FX) < fx_max) & (np.abs(FY) < fy_max)).astype(np.uint8),
                                dtype=dtype)
